...
// 		$(document).on("click",".tiptop a",function(){
// 			$(".tip").fadeOut(200);
// 		});
...
<!-- 			        	<th data-field="id" data-checkbox="true" data-valign="middle"></th> -->
			            ...
			            <th data-field="update_time" data-align="center"   data-valign="middle" data-width="120">修改时间</th>
...
<script>
		function addnews(){
			window.location.href = '${root}/manage/news_view?city=${city_code}';
		}
		$(function(){
// 			alert(1);
			initTable();
...
// 			html+='<button type="button" onclick="view(\''+row.id+'\')" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-search"></span> 预览  </button>';
		...
		function contentManage(id){
			window.location.href = '${root}/manage/news_content_list?n_id='+id;
		}
		function view(id){
			window.location.href = '${root}/manage/news_view?city=${city_code}&id='+id;
		}
		function pubOrDown(id,num){
			$.ajax({
			   type: "POST",
			   url:"${root}/manage/pubNewOrDown",
			   data:{
				   id:id,
				   status:num
			   },
