...
<body>
    ...
    <ul class="forminfo">
    <li><label >简介标题</label><input name="icon_value" type="text" class="dfinput" value="${menu.icon_value }"/><i>名称 不能超过30个字符</i></li>
    <li>
    	<label style="height:230px;">按钮</label>
		<input name="" id="checkfile1" ref-id='divimg2' type="button" class="btn" style="overflow:hidden" value="选择图片" onclick="clickImage(this)"/>
		<span id="divimg2" class="div_list divimg divli" style="background:	#F8F8F8;width:110px;height:90px;">
			<c:if test="${not empty menu}">
				<img alt="" src="${menu.icon }" width="100px;" height="80px;" >
				<input type="hidden" name="icon" value="${menu.icon }">
			</c:if>
		</span>
	</li>
    <li>
    	<label style="height:230px;">图片</label>
...
<!-- 			<img alt="" src="" width="350px;" height="190px;" > -->
			<c:if test="${not empty menu}">
				<img alt="" src="${menu.image }"  width="340px;" height="180px;">
...
function addElementImg(base64) {
	...
	if(IMGID == 'divimg1'){
		...
		html+='<input type="hidden" name="image" value="'+base64+'">'
	} else if(IMGID == 'divimg2') {
		html+='<img alt="" src="'+base64+'"  width="100px;" height="80px;">';
	
		html+='<input type="hidden" name="icon" value="'+base64+'">'
	}
	$("#"+IMGID).append(html);
}
