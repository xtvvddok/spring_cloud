...
<!-- 			        	<th data-field="id" data-checkbox="true" data-valign="middle"></th> -->
						...
						<th data-field="icon_value" data-align="center"  data-width="200" data-valign="middle">名称</th>
			        	<th data-field="icon" data-align="center" data-formatter="showicon"  data-width="120" data-valign="middle">按钮</th>
			        	<th data-field="image" data-align="center" data-formatter="showimg"  data-width="120" data-valign="middle">图片</th>
...
<script>
		function addnews(){
			window.location.href = '${root}/manage/menu_view?city=${city_code}';
		}
		$(function(){
// 			alert(1);
			initTable();
		...
		function initTable(){
		    ...
		    $("#tabletemp").bootstrapTable({  
		        ...
		        url: '${root}/manage/menu_page', //获取数据的Servlet地址  
		        striped: true,  //表格显示条纹  
...
// 			html+='<button type="button" onclick="view(\''+row.id+'\')" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-search"></span> 预览  </button>';
		...
		function contentManage(id){
			window.location.href = '${root}/manage/menu_list?n_id='+id;
		}
		function view(id){
			window.location.href = '${root}/manage/menu_view?city=${city_code}&id='+id;
		}
		function pubOrDown(id,num){
			$.ajax({
			   type: "POST",
			   url:"${root}/manage/pubMenuOrDown",
			   data:{
				   id:id,
				   status:num
			   },
