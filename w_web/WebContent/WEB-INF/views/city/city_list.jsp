...
<!-- 			        	<th data-field="id" data-checkbox="true" data-valign="middle"></th> -->
						...
						<th data-field="city_name" data-align="center"  data-width="200" data-valign="middle">城市</th>
			        	<th data-field="topimage" data-align="center" data-formatter="showimg"  data-width="120" data-valign="middle">头图</th>
			            <th data-field="summary" data-align="center" data-formatter="showtitle"  data-valign="middle" data-width="200">简介</th>
...
<script>
		function addnews(){
			window.location.href = '${root}/manage/city_view';
		}
		$(function(){
// 			alert(1);
			initTable();
		...
		function initTable(){
		    ...
		    $("#tabletemp").bootstrapTable({  
		        ...
		        url: '${root}/manage/city_page', //获取数据的Servlet地址  
		        striped: true,  //表格显示条纹  
...
// 			html+='<button type="button" onclick="view(\''+row.id+'\')" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-search"></span> 预览  </button>';
		...
		function contentManage(id){
			window.location.href = '${root}/manage/city_menu_list?n_id='+id;
		}
		function view(id){
			window.location.href = '${root}/manage/city_view?id='+id;
		}
		function pubOrDown(id,num){
			$.ajax({
			   type: "POST",
			   url:"${root}/manage/pubCityOrDown",
			   data:{
				   id:id,
				   status:num
			   },
